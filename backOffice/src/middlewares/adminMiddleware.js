...
import { setIsAdmin, addFlashMessage, setLoading } from 'src/actions/app';
...
const adminMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    ...
    case SUBMIT_NEW_ADMIN_APPOINTMENT: {
      const { doctorToken, doctorId } = sessionStorage;
      if (!store.getState().admin.newAppointmentIsRecurent) {
        const data = {
        ...
        const startHour = new Date(store.getState().admin.newAppointmentStart).getHours();
        const startMinute = new Date(store.getState().admin.newAppointmentStart).getMinutes();
        const endHour = new Date(store.getState().admin.newAppointmentEnd).getHours();
        const endMinute = new Date(store.getState().admin.newAppointmentEnd).getMinutes();

        const startDate = Date.parse(store.getState().admin.startRecurenceDay);
        const endDate = Date.parse(store.getState().admin.endRecurenceDay);

        for (let currentDate = startDate; currentDate < endDate; currentDate += 86400000) {
          let appointment = false;
          const currentDateObject = new Date(currentDate);
          if (currentDateObject.getDay() === 1 && monday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 2 && tuesday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 3 && wednesday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 4 && thursday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 5 && friday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 6 && saturday) {
            appointment = true;
          }
          else if (currentDateObject.getDay() === 0 && sunday) {
            appointment = true;
          }
          if (appointment) {
            const start = new Date();
            start.setDate(currentDateObject.getDate());
            start.setMonth(currentDateObject.getMonth());
            start.setFullYear(currentDateObject.getFullYear());
            start.setHours(startHour);
            start.setMinutes(startMinute);
            const end = new Date();
            end.setDate(currentDateObject.getDate());
            end.setMonth(currentDateObject.getMonth());
            end.setFullYear(currentDateObject.getFullYear());
        ...
        store.dispatch(getAllDoctorAppointments());
        store.dispatch(setAdminPageFields('openModal', false));
        store.dispatch(addFlashMessage('Les rendez-vous ont bien été enregistrés', 'success'));
      }
      break;
    }
    case GET_ALL_DOCTOR_APPOINTMENTS: {
      const { doctorToken, doctorId } = sessionStorage;
      if (store.getState().main.isAdmin) {
        axios({
          method: 'get',
    ...
    case SUBMIT_ADMIN_CONNECT_FORM: {
      const data = {
        email: store.getState().admin.emailValue.trim().toLowerCase(),
